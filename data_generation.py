...
class EventDataset(Dataset):
    def __init__(self, samples_folder,  # Folder from where to load the data
        ...
        self.sparse_frame_len_us = int(
            self.samples_folder.split('/')[-3].split('_')[-1])  # len of each loaded sparse frame
        ...
        if min_activations_per_patch > 0 and min_activations_per_patch <= 1:
            ...
            self.min_activations_per_patch = 0
        ...
        if dataset_name == 'DVS128':
            ...
            self.labels = np.array([int(t[5:7]) for s in self.samples for t in s.split('_') if 'label' in t]).astype(
                'int8')
        ...
        elif dataset_name in ['ASL_DVS', 'HMDB', 'UCF101', 'UCF50', 'SLAnimals_4s', 'SLAnimals_3s', 'N_Cars',
                              'Caltech']:
            ...
            raise ValueError(f'dataset_name [{dataset_name}] not handled')
    ...
    def crop_in_space(self, total_events):
        ...
        if not self.validation:  # Crop sequence randomly
            ...
            x_init = np.random.randint(x_size - new_x_size + 1);
            x_end = x_init + new_x_size
            y_init = np.random.randint(y_size - new_y_size + 1);
            ...
            x_init = (x_size - new_x_size) // 2;
            x_end = x_init + new_x_size
            y_init = (y_size - new_y_size) // 2;
            y_end = y_init + new_y_size
            total_events = total_events[:, y_init:y_end, x_init:x_end, :]
    ...
    def get_label_dict(self):
        ...
        for k in label_dict:
            label_dict[k] = torch.IntTensor(label_dict[k])
        return label_dict
    ...
    def __getitem__(self, idx, return_sparse_array=False):
        ...
        if 'random_frame_size' in self.augmentation_params and self.augmentation_params[
            'random_frame_size'] is not None:
            total_events = self.crop_in_space(total_events)

        if not self.validation and self.h_flip and np.random.rand() > 0.5:
        ...
        while sf_num >= 0:
            ...
            if current_chunk is None:
                ...
                if '1' in self.preproc_polarity:
                    current_chunk = current_chunk.sum(-1, keepdims=True)
            else:
            ...
            for ib_num, i in enumerate(list(range(0, bins_init, bins_step))[:self.bins]):
                if ib_num == self.bins - 1:
                    step = 99999
                else:
                    step = bins_step
                chunk_candidate.append(current_chunk[i:i + step].sum(0))
            chunk_candidate = np.stack(chunk_candidate, axis=-1).astype(float)
            chunk_candidate = chunk_candidate.reshape(chunk_candidate.shape[0], chunk_candidate.shape[1],
                                                      chunk_candidate.shape[2] * chunk_candidate.shape[3])
            ...
            inds = (polarity.sum(-1) != 0).reshape(polarity.shape[0], polarity.shape[1],
            ...
            if self.min_patches_per_chunk and inds.sum() < self.min_patches_per_chunk:
                continue

            # Reshape to (num_patches x token_dim)
            polarity = polarity.reshape(polarity.shape[0] * polarity.shape[1],
                                        self.patch_size * self.patch_size * self.preproc_event_size)  # self.token_dim
            # Get pixel locations
            ...
            if not self.validation and len(inds) > 0 and 'drop_token' in self.augmentation_params and \
                    self.augmentation_params['drop_token'][0] != 0.0:
                inds = np.random.choice(inds, replace=False,
                                        size=max(1, int(len(inds) * (1 - self.augmentation_params['drop_token'][0]))))
            polarity, pixels = polarity[inds], pixels[inds]

            if 'log' in self.preproc_polarity:
                polarity = np.log(polarity + 1)
            else:
                raise ValueError('Not implemented', self.preproc_polarity)
...
class CustomBatchSampler():
    ...
    def __len__(self):
        epoch_length = sum(sum([len(v) for v in self.label_dict.values()]) * self.sample_repetitions // self.batch_size) * self.sample_repetitions // self.batch_size
        return epoch_length
        ...
        while True:
            ...
            for b in range(self.batch_size // self.sample_repetitions):
                if len(total_labels) == 0:
                    total_labels = self.unique_labels.copy()
                ...
                for _ in range(self.sample_repetitions):
                    inds.append(ind)
    ...
    for num_sample, action_sample in enumerate(samples):
        ...
        for chunk_num, chunk in enumerate(action_sample):
            ...
            if chunk_events == 0:
                continue
            if pre_padding:
                batch_data[-(num_chunks - chunk_num), num_sample, -chunk_events:, :] = chunk
            else:
                batch_data[chunk_num, num_sample, :chunk_events, :] = chunk
...
class Event_DataModule(LightningDataModule):
    def __init__(self, batch_size, chunk_len_ms,
                 ...
                 custom_sampler=True,
                 workers=0, pin_memory=False, classes_to_exclude=[], balance=None):
        super().__init__()
        self.batch_size = batch_size
        self.chunk_len_ms = chunk_len_ms
        self.patch_size = patch_size
        ...
        if dataset_name == 'DVS128':
            ...
            raise ValueError(f'Dataset [{dataset_name}] not handled')
    ...
    def train_dataloader(self):
        ...
        if self.custom_sampler:
            ...
            dl = DataLoader(dt, batch_sampler=sampler, collate_fn=self.custom_collate_fn, num_workers=self.workers,
                            pin_memory=self.pin_memory)
        else:
            dl = DataLoader(dt, batch_size=self.batch_size, collate_fn=self.custom_collate_fn, shuffle=True,
    ...
    def val_dataloader(self):
        dt = EventDataset(self.data_folder + 'test/', chunk_len_ms=self.chunk_len_ms,
                          ...
                          augmentation_params=self.augmentation_params,
                          classes_to_exclude=self.classes_to_exclude)
        dl = DataLoader(dt, batch_size=(self.batch_size // 2) + 1, shuffle=False, collate_fn=self.custom_collate_fn,
                        num_workers=self.workers, pin_memory=self.pin_memory)
        return dl
