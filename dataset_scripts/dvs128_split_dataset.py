...
def store_samples(events_files, mode):
    for events_file in tqdm(events_files):
        ...
        for _, row in labels.iterrows():
            if (row.startTime_usec - 1 - prev_event) > 0:
                time_segment_class.append((prev_event, row.startTime_usec - 1, 0, (row.startTime_usec - 1 - prev_event) / 1000))
            if ((row.endTime_usec - row.startTime_usec)) > 0:
                time_segment_class.append((row.startTime_usec, row.endTime_usec, row['class'], (row.endTime_usec - row.startTime_usec) / 1000))
        ...
        for i, e in enumerate(events):
            if e[2] >= time_segment_class[0][1]:
                # Store event
                if len(curr_event) > 1:
                    total_events.append((time_segment_class[0], len(curr_event), np.array(curr_event)))
                else:
                    print(' ** EVENT ERROR:',
                          events_file.replace('.aedat', '') + '_num{:02d}_label{:02d}.pckl'.format(i,
                                                                                                   time_segment_class[
                                                                                                       0][2]),
                          time_segment_class[0], len(curr_event))
        ...
        for i, (meta, _, events) in enumerate(total_events):
            t = os.path.join(path_dataset_dst, mode,
                             events_file.replace('.aedat', '') + '_num{:02d}_label{:02d}.pckl'.format(i, meta[2]))
            pickle.dump((events, meta[2]), open(t, 'wb'))
